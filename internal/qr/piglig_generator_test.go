...
func TestPigligGeneratorImage(t *testing.T) {
	t.Parallel()

	g := NewPigligGenerator(320)
...
func TestPigligGeneratorImageWithIcon(t *testing.T) {
	t.Parallel()
	...
	for y := 0; y < 32; y++ {
		for x := 0; x < 32; x++ {
			icon.Set(x, y, color.RGBA{R: 220, G: 30, B: 30, A: 255})
		}
	}
